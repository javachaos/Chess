...
use chess_engine::{Color, Game, GameStatus, Move, Piece, PieceKind, SearchConfig, SearchResult, Square};
use eframe::egui::{
    self, Align2, Color32, FontId, IconData, RichText, Stroke, StrokeKind, Vec2, ViewportBuilder,
};
...
impl ChessApp {
    ...
    fn ai_progress(&self) -> Option<f32> {
        self.ai_task.as_ref().map(|task| {
            (task.started_at.elapsed().as_secs_f32() / task.time_limit.as_secs_f32()).clamp(0.0, 1.0)
        })
    }
                                    ...
                                    for file in files {
                                        ...
                                        let is_last_move_square =
                                            self.last_move.is_some_and(|chess_move| {
                                                chess_move.from == square || chess_move.to == square
                                            });
                                        ...
                                        if is_selected {
                                            ui.painter().rect_stroke(
                                                ...
                                                Stroke::new(2.0, Color32::from_rgb(180, 134, 24)),
                                                StrokeKind::Inside,
                                            );
                                        }
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::SidePanel::right("controls")
            ...
            .show(ctx, |ui| {
                ...
                ui.label(
                    "Depth is a ceiling. The AI stops early when it hits the time limit.",
                );
